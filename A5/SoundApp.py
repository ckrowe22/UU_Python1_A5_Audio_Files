...
# Sound Editor GUI
# Written by David E. Johnson for U of Utah COMP1010
...
class SoundApp:
    def __init__(self):
        ...
        self.canvas = turtle.ScrolledCanvas(self.frame, 500, 500, len(self.original_samples), 500)
        self.canvas.pack()
    ...
    def load_file(self, filename):
        ...
        self.original_samples = sound_data[0]
    ...
    def reduce_and_visualize(self):
        skip = 3 # use every third sample
        new_samples = make_reduced_samples(self.original_samples, skip)
        self.play(new_samples, self.original_sample_rate//skip)
        stretched_samples = [item for item in new_samples for i in range(skip)]
        self.sample_viewer.drawSamples(stretched_samples, self.original_samples, 300)

    def make_clipped_samples_and_visualize(self):
        new_samples = make_clipped_samples(self.original_samples, 2000)
        self.play(new_samples, self.original_sample_rate)
        self.sample_viewer.drawSamples(new_samples, self.original_samples, 300)
    ...
    def make_tone_samples_and_visualize(self):
        new_samples = make_tone()
        self.play(new_samples, self.original_sample_rate)
        self.sample_viewer.drawSamples(new_samples, self.original_samples, 300)
    ...
    def add_buttons(self, button_frame):
        ...
        reduce_button = tk.Button(button_frame,
                       text="Reduce",
                       fg="red",
                       command = self.reduce_and_visualize)
        reduce_button.pack(side=tk.TOP)
        ...
        tone_button = tk.Button(button_frame,
                       text="Make Tone",
                       fg="red",
                       command = self.make_tone_samples_and_visualize)
        tone_button.pack(side=tk.TOP)
